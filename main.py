...
@app.post("/comprar_accesorio/")
def comprar_accesorio(compra: CompraAccesorio):
    # Obtener datos del accesorio desde la tienda
    accesorio_response = requests.get(f"{STORE_API_URL}/producto/{compra.accesorio_id}")
    ...
    # Verificar que el accesorio tiene un tipo (sombrero, mano, etc.)
    ...
    # Asignar accesorio al rockie del estudiante
    ...
    rockie[accesorio_tipo] = compra.accesorio_id
...
def crear_objeto(objeto: ObjetoTienda):
    # Crear el objeto en la tienda
    objeto_response = requests.post(f"{STORE_API_URL}/producto/", json=objeto.dict())
    ...
    # Si es accesorio, guardarlo en el microservicio de rockies
...
def completar_actividad(student_id: int, activity: CompletarActividad):
    # Marcar actividad como completada (simulación)
    ...
    # Aumentar los RockieCoins del estudiante
    student['RockieCoins'] += 10  # Aumenta 10 monedas por actividad completada

    # Actualizar los RockieCoins del estudiante
